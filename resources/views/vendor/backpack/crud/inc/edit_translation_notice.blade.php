    @php
        ...
        if($showTranslationNotice) {
            ...
            if(count($translatedLocales) === 1) {
                $translationNoticeText .= '<a href="'.url($crud->route.'/'.$entry->getKey().'/edit').'?_locale='.$editLocale.'&_fallback_locale='.current($translatedLocales).'" class="text-text-50"> > '.trans('backpack::crud.no_attributes_translated_href_text', ['locale' => $translatableLocales[current($translatedLocales)]]).'</a>';
            }else {
                foreach($translatedLocales as $locale) {
                    $translationNoticeText .= '<a href="'.url($crud->route.'/'.$entry->getKey().'/edit').'?_locale='.$editLocale.'&_fallback_locale='.$locale.'" class="text-text-50"> > '.trans('backpack::crud.no_attributes_translated_href_text', ['locale' => $translatableLocales[$locale]]).' </a><br/>';
                }
            }
        }
    @endphp
