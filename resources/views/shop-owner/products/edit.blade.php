...
<div class="container mx-auto px-4 py-16 max-w-5xl">
    <div class="flex items-center justify-between mb-10">
        <div>
            <h1 class="text-4xl font-black text-text-950 mb-2">{{ __('edit_product') }}</h1>
            <p class="text-text-500 font-medium">{{ __('update_details_pricing_and_inventory_levels') }}</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <div class="bg-background-50 border border-background-200 rounded-[2.5rem] p-8 md:p-10 shadow-xl shadow-background-900/5">
                    <div class="mb-6">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" required 
                               class="w-full px-5 py-4 rounded-2xl border border-background-200 bg-background-100/50 text-text-950 focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 outline-none transition-all font-bold">
                        @error('name')
                    ...
                    <div class="mb-0">
                        ...
                        <textarea name="description" id="description" rows="6" required 
                                  class="w-full px-5 py-4 rounded-2xl border border-background-200 bg-background-100/50 text-text-950 focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 outline-none transition-all leading-relaxed">{{ old('description', $product->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-accent-600 font-bold ml-1">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="price" id="price" value="{{ old('price', $product->price) }}" step="0.01" required 
                                       class="w-full pl-10 pr-5 py-4 rounded-2xl border border-background-200 bg-background-100/50 text-text-950 focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 outline-none transition-all font-black text-xl">
                            </div>
                        </div>
                        <div>
                            ...
                            <input type="number" name="stock" id="stock" value="{{ old('stock', $product->stock) }}" required 
                                   class="w-full px-5 py-4 rounded-2xl border border-background-200 bg-background-100/50 text-text-950 focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 outline-none transition-all font-black text-xl">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                <div class="bg-background-50 border border-background-200 rounded-[2.5rem] p-6 shadow-xl shadow-background-900/5">
                    ...
                    <div x-data="{ fileName: '' }">
                        <input type="file" name="image" id="image" accept="image/*" class="hidden" @change="fileName = $event.target.files[0].name">
                        <label for="image" class="flex flex-col items-center justify-center w-full py-6 border-2 border-dashed border-background-200 rounded-2xl cursor-pointer hover:border-primary-500 hover:bg-primary-50/10 transition-all group">
                            ...
                            <span class="text-[10px] font-black uppercase tracking-widest text-text-500" x-text="fileName ? fileName : '{{ __('change_image') }}'"></span>
                        </label>
                    </div>
                    @error('image')
                        <p class="mt-2 text-sm text-accent-600 font-bold ml-1">{{ $message }}</p>
